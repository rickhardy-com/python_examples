...
def dijkstras_pathfinder (graph, start):
    
    # Accepts a weighted graph (dictionary of dictionaries) as input
    # for every vertex creates entries in two dictionaries, previous_vertex and distance
    # in this implementation, only previous_vertex is output
    # distance (or cost) is still required to calculate the shortest path
    ...
    while graph:
        ...
        # For the vertex found in the previous step, examine each of its neighbours
...
if __name__ == "__main__":  
    ...
    start = "A"
    previous_vertex = dijkstras_pathfinder(graph, "A")
