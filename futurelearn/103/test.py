...
def insert_result (card, winner):
    print (card)
    check_for_incomplete_result = "SELECT card1 FROM result where card2 = 'None'"
    first_card = conn.execute(check_for_incomplete_result)
    first_card_name = first_card.fetchone()
    print ('first: ',result)
    if first_card :
        print ('Yes')
        print ('second: ', card)
        winning_card = card if winner == 'y' else first_card_name       
        update_sql = "UPDATE result SET card2 = '{}', winner = '{}' ".format(card, winning_card)
        conn.execute(update_sql)
        conn.commit()
    else: 
        print ('No')
